...
// Function to execute the tool calls made by the model
        ...
        if (!toolCalls || toolCalls.length === 0) {
            ...
            try {
                ...
                if (parsedContent.name && parsedContent.arguments) {
                    ...
                    const result = await available_functions[functionName](
                ...
                const functionCallMatch = content.match(/(\w+)\(([^)]+)\)/);
                if (functionCallMatch) {
                    ...
                    const result = await available_functions[functionName](...args);
